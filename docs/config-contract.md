...
Source: [`canshift-firmware/src/config/config_types.h`](https://github.com/CANShift/canshift-firmware/blob/main/src/config/config_types.h)
...
| `device.json`         | CAN speed; TWAI pins only as an override (the board profile owns them) | Same — wired host-side in studio-web (#1118) via `CMD_GET_DEVICE_CONFIG` (0x03) / `CMD_PUT_DEVICE_CONFIG` (0x04) |
...
ConfigLoader::loadAll() reads each canonical config
    ...
    └── Parses device.json → CfgDevice struct (CAN speed; pins left unset fall back to the board profile)
    │
    ▼
